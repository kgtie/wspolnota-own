...
class EditNewsComment extends EditRecord
{
    ...
    protected function mutateFormDataBeforeSave(array $data): array
    {
        ...
        if ((bool) ($data['is_hidden'] ?? false)) {
            $data['hidden_at'] = $this->getRecord()->hidden_at ?? now();
            $data['hidden_by_user_id'] = $this->getRecord()->hidden_by_user_id ?? ($actor instanceof User ? $actor->getKey() : null);
        } else {
            $data['hidden_at'] = null;
            $data['hidden_by_user_id'] = null;
        }
    ...
    protected function restoreVisibilityAction(): Action
    {
        return Action::make('restore_visibility')
            ...
            ->visible(fn (): bool => $this->getRecord()->is_hidden)
            ->action(function (): void {
                $this->getRecord()->restoreVisibility();
    ...
    protected function deleteCommentAction(): Action
    {
        return Action::make('delete_comment')
            ->label('Usun komentarz')
            ...
            ->action(function (): void {
                ...
                $this->redirect(NewsCommentResource::getUrl('index'));
            });
    }
}
