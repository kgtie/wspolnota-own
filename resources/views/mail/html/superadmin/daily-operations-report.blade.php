<div style="font-family:Arial,Helvetica,sans-serif;">
    ...
    <h1 style="margin:10px 0 16px;font-family:Georgia,'Times New Roman',serif;font-size:30px;line-height:1.15;color:#1c1917;">Dobowy raport operacyjny Wspólnoty.</h1>
    ...
    <div style="margin:0 0 18px;padding:16px 18px;border-radius:18px;background-color:#f8f3ec;border:1px solid #eadfce;">
        <div style="font-size:12px;letter-spacing:0.12em;text-transform:uppercase;color:#8e5520;font-weight:700;">Podsumowanie</div>
        <div style="margin-top:10px;font-size:15px;line-height:1.8;color:#57534e;">
            <strong>Nowi użytkownicy:</strong> {{ $report['overview']['new_users'] }}<br>
            <strong>Nowe parafie:</strong> {{ $report['overview']['new_parishes'] }}<br>
            <strong>Opublikowane aktualności:</strong> {{ $report['overview']['published_news'] }}<br>
            <strong>Opublikowane ogłoszenia:</strong> {{ $report['overview']['published_announcements'] }}<br>
            ...
            <strong>Push wysłane / błędne:</strong> {{ $report['overview']['push_sent'] }} / {{ $report['overview']['push_failed'] }}<br>
            <strong>Nieudane zadania:</strong> {{ $report['overview']['failed_jobs'] }}<br>
            <strong>Wpisy aktywności:</strong> {{ $report['overview']['activity_entries'] }}<br>
    ...
    @if (! empty($report['users']['items']))
        <div style="margin:0 0 18px;padding:16px 18px;border-radius:18px;background-color:#fffdf9;border:1px solid #eadfce;">
            <div style="font-size:20px;line-height:1.35;color:#1f2937;font-weight:700;">Najważniejsze nowe konta</div>
    ...
    @if (! empty($report['content']['news']) || ! empty($report['content']['announcements']) || ! empty($report['content']['masses_created']))
        <div style="margin:0 0 18px;padding:16px 18px;border-radius:18px;background-color:#fffdf9;border:1px solid #eadfce;">
            <div style="font-size:20px;line-height:1.35;color:#1f2937;font-weight:700;">Treści i liturgia</div>
            <div style="margin-top:10px;font-size:14px;line-height:1.8;color:#57534e;">
                <strong>Aktualności:</strong> {{ $report['overview']['published_news'] }}<br>
                <strong>Ogłoszenia:</strong> {{ $report['overview']['published_announcements'] }}<br>
    ...
    @if (! empty($report['system']['failed_jobs']))
        <div style="margin:0 0 18px;padding:16px 18px;border-radius:18px;background-color:#fff1f2;border:1px solid #fecdd3;">
            <div style="font-size:20px;line-height:1.35;color:#9f1239;font-weight:700;">Nieudane zadania</div>
            <div style="margin-top:10px;font-size:14px;line-height:1.8;color:#881337;">
                @foreach ($report['system']['failed_jobs'] as $job)
                    {{ $job['failed_at'] }} | kolejka: {{ $job['queue'] }} | {{ $job['exception'] }}<br>
                @endforeach
            </div>
        </div>
    @endif
    ...
    <p style="margin:0;font-size:15px;line-height:1.8;color:#57534e;">
        To raport zbiorczy z całości danych, które backend Wspólnoty obecnie śledzi i zapisuje.
    </p>
</div>
