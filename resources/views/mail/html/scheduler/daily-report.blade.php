<div style="font-family:Arial,Helvetica,sans-serif;">
    ...
    <h1 style="margin:10px 0 18px;font-family:Georgia,'Times New Roman',serif;font-size:30px;line-height:1.15;color:#1c1917;">Raport schedulera.</h1>
    ...
    @foreach ($report['jobs'] as $job)
        <div style="margin-bottom:18px;padding:16px 18px;border-radius:18px;background-color:#fffdf9;border:1px solid #eadfce;">
            ...
            <div style="margin-top:10px;font-size:14px;line-height:1.8;color:#57534e;">
                ...
                <strong>Zakonczone przebiegi:</strong> {{ $job['completed_runs'] }}
            </div>
            ...
            @if ($job['latest_error'])
                <div style="margin-top:12px;font-size:14px;line-height:1.7;color:#9f1239;">
                    <strong>Ostatni blad:</strong> {{ $job['latest_error'] }}
                </div>
            @endif
        </div>
    @endforeach
    ...
    <p style="margin:0;font-size:15px;line-height:1.8;color:#57534e;">
        @if ($report['has_failures'])
            W raporcie wykryto bledy. Sprawdz logi aplikacji i activity log.
        @else
